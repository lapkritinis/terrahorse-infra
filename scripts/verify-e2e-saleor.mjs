...
try {
  ...
  if (!variant || variant.stocks.length !== 1 || variant.stocks[0].warehouse.slug !== 'terrahorse-e2e' || variant.stocks[0].quantity !== 100 || !variant.channelListings.some((listing) => listing.channel.slug === 'terrahorse-e2e' && listing.price.amount === 10 && listing.price.currency === 'EUR')) throw new Error('e2e-variant-not-isolated')
  ...
  process.stdout.write('E2E Saleor checkout delivery verified.\n')
} finally {
  if (checkoutId) await call(`mutation($id: ID!) { checkoutDelete(id: $id) { errors { code } } }`, { id: checkoutId })
}
